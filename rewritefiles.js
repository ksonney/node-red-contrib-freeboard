...
fs.readFile('/app/node_modules/freeboard/freeboard/index.html', 'utf8', function(err, data) {
    if (err) {
        return console.log(err);
    }
    var result = data.replace(/head.js[\s\S]*?<\/script>/g, head);
    fs.writeFile('/app/node_modules/freeboard/freeboard/index.html', result, 'utf8', function(err) {
...
fs.readFile('/app/node_modules/freeboard/freeboard/js/freeboard.js', 'utf8', function(err, data) {
    if (err) {
        return console.log(err);
    }
    var result = data.replace(/this\.saveDashboard =[\s\S]*?a\.click[\s\S]*?\}/g, saveDashboard);
    fs.writeFile('/app/node_modules/freeboard/freeboard/js/freeboard.js', result, 'utf8', function(err) {
        if (err) return console.log(err);
    });
});

// Copy the plugins across
fs.createReadStream('freeboard-widget-rag-files/jquery.keyframes.min.js').pipe(fs.createWriteStream('/app/node_modules/freeboard/freeboard/plugins/thirdparty/jquery.keyframes.min.js'));
fs.createReadStream('freeboard-widget-rag-files/widget.ragIndicator.js').pipe(fs.createWriteStream('/app/node_modules/freeboard/freeboard/plugins/thirdparty/widget.ragIndicator.js'));